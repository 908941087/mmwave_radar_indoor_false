...
def generate_img_for_pcds(path):
    import glob

    pcd_file_paths = glob.glob(path + "/*.pcd")
    for pcd_file_path in pcd_file_paths:
        name = pcd_file_path[pcd_file_path.rfind("/") + 1 : pcd_file_path.find(".")] + ".png"
        points = get_points_from_pcd(pcd_file_path)
        xy_lim = get_xy_lim(points)
        ratio = (xy_lim[3] - xy_lim[2]) / float(xy_lim[1] - xy_lim[0])
        scale = 15
        fig = plt.figure(figsize=(scale * ratio, scale), facecolor=(0.1882, 0.1882, 0.1882, 1))
        plt.axis('off')
        ax = fig.add_subplot(111)
        ax.scatter([p[0] for p in points], [p[1] for p in points], c='r', s=1)
        ax.set_facecolor((0.1882, 0.1882, 0.1882, 1))
        plt.savefig(name, dpi=300, facecolor=fig.get_facecolor())

if __name__ == '__main__':
    path = "ti_ws/src/py_interface/scripts/EnvClassifier/pcds/paper_data"
    generate_img_for_pcds(path)