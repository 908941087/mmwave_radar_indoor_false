...
class AmclNode
{
  public:
    ...
    boost::thread *computeCurrentScoreThread;
...
main(int argc, char** argv)
{
  ...
  ROS_WARN("AMCL: Start AMCL");
  std::cout << "AMCL: Start AMCL" << std::endl;
  if (argc == 1)
  {
    // run using ROS input
    ros::spin();
        ...
        first_reconfigure_call_(true)
{
  ...
  computeCurrentScoreThread = new boost::thread(&AmclNode::computeScoreThread,this);
...
void AmclNode::runFromBag(const std::string &in_bag_fn)
{
  ...
  BOOST_FOREACH(rosbag::MessageInstance const msg, view)
  {
    ...
    ros::getGlobalCallbackQueue()->callAvailable(ros::WallDuration());
      ...
      ROS_INFO("AMCL: Start process global localization");
      tf2_msgs::TFMessage::ConstPtr tf_msg = msg.instantiate<tf2_msgs::TFMessage>();
    if (tf_msg != NULL)
    {
      ROS_INFO("AMCL: Publish amcl tf form map to odom");
...
// Thread for print match score of scan an map
void AmclNode::computeScoreThread(){
    ...
    while(1){
        ...
        sleep(0.2);
        if(count>5){
            std::cout << "AMCL pf socre: " << sum_score << std::endl;
            count =0;
            sum_score = 0.0;
        }
    }
